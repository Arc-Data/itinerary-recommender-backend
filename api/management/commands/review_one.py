...
class Command(BaseCommand):
    help = 'Create or update reviews for a location with 1-star ratings'
    ...
    def handle(self, *args, **options):
        ...
        for user_id in user_ids:
            ...
            review, created = Review.objects.get_or_create(user=user, location=location, defaults={'rating': 1})
            review.comment = f"This is a test review."
            review.rating = 1
            review.datetime_created = timezone.now()
            review.save()

            self.stdout.write(self.style.SUCCESS(f"Review by {user.get_full_name()} created/updated for location '{location.name}' with a 1-star rating"))